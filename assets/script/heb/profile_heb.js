...
profileHeb.init = function () {
    this.registAllMessage();
    this.initRoomData();
};

profileHeb.initRoomData = function () {
    this.initCreateRoomOpts();
    profileHeb.roomInfo = {};
    profileHeb.gameInfo = {};
    profileHeb.resultData = {};
};

profileHeb.initCreateRoomOpts = function () {

    var data = cc.sys.localStorage.getItem('createRoomOpts_heb');
    if (data != null) {
        profileHeb.createRoomOpts = JSON.parse(data);
        return;
    }
    profileHeb.createRoomOpts = {};
    // 房间类型（默认普通房间）1：普通房间 2：俱乐部房间
    // 回合类型1：4局 2：8局 3：12局 64：1圈 65：2圈 66：4圈
    ...
    profileHeb.createRoomOpts.bossType = 1;
    //局数
    profileHeb.createRoomOpts.roundMax = 4;
    // 人数
    profileHeb.createRoomOpts.joinermax = 2;
    /** 点炮包三家 */
    profileHeb.createRoomOpts.dianpao = 0;
    //小鸡飞蛋
    //profileHeb.createRoomOpts.xiaojifeidan = 0;
    //下蛋算站立
    //profileHeb.createRoomOpts.xiadanzhanli = 0;
    /** 带缺门 */
    profileHeb.createRoomOpts.quemen = 0;
    /** 对宝翻倍 */
    profileHeb.createRoomOpts.duibaofanbei = 0;
    //明宝&暗宝 0  1
    profileHeb.createRoomOpts.baotype = 0;
    //金币模式等级
    profileHeb.createRoomOpts.scoreLv = 0;
};

profileHeb.saveCreateRoomOpts = function () {
    if (profileHeb.createRoomOpts == null || profileHeb.createRoomOpts == undefined) return;
    cc.sys.localStorage.setItem('createRoomOpts_heb', JSON.stringify(profileHeb.createRoomOpts));
};

profileHeb.registAllMessage = function () {
    console.log("----- 初始化哈尔滨消息");
    // 游戏开局
    ...
    GameNet.getInstance().setCallBack('onHuaDianMahjongReconnecet', function (data) {
        WriteLog("~~~~~~~~~~~onHuaDianMahjongReconnecetHeb :~~~~~~~~~~~~~~" + JSON.stringify(data));
        GameData.game =  GameData.game || GameData.initGameData();
        GameData.game.gameStart = true;
        GameData.game.cardleft = data.remainCardCount;  //剩余手牌的数量
        GameData.game.initcards = true;
...
profileHeb.unregistAllMessage = function () {
    console.log("----- 注销哈尔滨消息");
...
//返回玩家的操作列表
profileHeb.getPlayerOperationsByUid = function(uid){
...
//返回玩家的听牌状态
profileHeb.getTingStateByUid = function(uid){
...
//返回小结算得分描述
profileHeb.getResultScoreInfoByUid = function(uid){
...
profileHeb.getGangCardByUid = function (uid,GangType) {
...
//数组去重
profileHeb.removerepeat = function (array){
    array.sort(); //先排序
    var res = [array[0]];
 ...
 */
profileHeb.guolv = function(array1,array2){
    var array =[];
...
/*
 判断数组元素是否相等
 */
profileHeb.isAllEqual =function (array) {
...
//判断数组是否有2个幺鸡
profileHeb.isHasTwoJiToHandcard = function () {
...
profileHeb.summaryDataByUid = function(uid) {
...
profileHeb.checkIsTingPlayer = function() {
    var i = 0;
    for (var key in GameData.cards) {
        if (GameData.cards[key]['tingState'] == 1 || GameData.cards[key]['tingState'] == 2) {
            return true;
...
//轮到非下家操作时提示仍然在本家显示
//轮到下家操作自然而然轮到下家 mmp ~&~!
profileHeb.getTurnByUid = function () {
    var isNextPlayer = profileHeb.isNextPlayer(GameData.game.lastdisUid,GameData.game.turn);
    if (isNextPlayer) {
...
profileHeb.isNextPlayer = function (CurrUid,nextUid) {
    if (CurrUid == null) {
        return true;
    }
 ...
 * @returns {number}
 */
profileHeb.getCardIndexByUid = function (uid1,uid2) {
...
profileHeb.getGangSorce = function(gangArray) {
    var score = 0;
