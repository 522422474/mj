...
cc.Class({
    ...
    onLoad: function () {
        ...
        require('util').registEvent('tjddz-onReady', this, this.onReady);
        require('util').registEvent('onCreatorQuit', this, this.onCreatorQuit);
        require('util').registEvent('tjddz-onDiZhu', this, this.showPlayers);
        ...
        require('util').registEvent('tjddz-onGameAllScore', this, this.onGameAllScoreHandler);
        require('util').registEvent('tjddz-onGameStart', this, this.onGameStartHandler);
    ...
    onDestroy: function () {
        ...
        unrequire('util').registEvent('tjddz-onReady', this, this.onReady);
        unrequire('util').registEvent('onCreatorQuit', this, this.onCreatorQuit);
        unrequire('util').registEvent('tjddz-onDiZhu', this, this.showPlayers);
        ...
        unrequire('util').registEvent('tjddz-onGameAllScore', this, this.onGameAllScoreHandler);
        unrequire('util').registEvent('tjddz-onGameStart', this, this.onGameStartHandler);
        unrequire('util').registEvent('onRoomReadyInfo', this, this.onRoomReadyInfoHandler);
        unrequire('util').registEvent('tjddz-onGameScore', this, this.onGameScoreHandler);
    ...
    onGameAllScoreHandler: function (data) {
        for (var i = 0; i < this.headers.length; i++) {
            ...
            if(gameDefine.currencyType.Currency_Coin == GameData.room.opts.currencyType){
                ...
                var score = GameDataTJDDZ.allScores[uid] == undefined ? 0 : GameDataTJDDZ.allScores[uid];
    ...
    onPrepareInfo: function(data) {
        ...
        {
            if (GameDataTJDDZ.game.onRoomReadyInfo[key] == true)
            {
    ...
    showPlayer: function (direction, parent) {
        ...
        if (player != null) { //Disconnect
            ...
            if(gameDefine.currencyType.Currency_Coin == GameData.room.opts.currencyType){
                ...
                var score = 0;
                if (GameDataTJDDZ.roomsummaryData.score != undefined) {
                    score = GameDataTJDDZ.roomsummaryData.score[player.uid] == undefined ? 0 : GameDataTJDDZ.roomsummaryData.score[player.uid];
                }
            ...
            if (GameDataTJDDZ.game.dizhuUid != 0) {
    ...
    showKicking: function() {
        for (var key in GameDataTJDDZ.kicking.kicking)
        {
            ...
            if (pos == 'down')
            {
                this.showPlayerTiChuai(this.play_down_player, GameDataTJDDZ.kicking.dizhu, key);
            }
            else if (pos == 'right')
            {
                this.showPlayerTiChuai(this.play_right_player, GameDataTJDDZ.kicking.dizhu, key);
            }
            if (pos == 'left')
            {
                this.showPlayerTiChuai(this.play_left_player, GameDataTJDDZ.kicking.dizhu, key);
            }
        }
    },
    ...
    backBtnClicked: function () {
        if (GameData.room.creator == GameData.player.uid) {
            ...
            TJDDZHandler.getInstance().quitMjRoom(GameData.room.id);
        }
    },
    ...
    getInviteStr: function () {
        ...
        if (GameData.room.opts.jiaofenType == 1)
        {
            str += "赢家叫分,";
        }
        else if (GameData.room.opts.jiaofenType == 2)
        {
            str += '轮庄叫分,';
        }
        if (GameData.room.opts.mud == true)
        {
            str += "带泥儿,";
        } 
