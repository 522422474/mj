...
cc.Class({
    ...
    showPlayer: function (player, idx) {
        ...
        if (GameDataTJDDZ.roomsummaryData && Object.keys(GameDataTJDDZ.roomsummaryData).length != 0) {
            if (Object.keys(GameDataTJDDZ.roomsummaryData.maxFen).length != 0) {
                maxScore.getComponent("cc.Label").string = '单局最高分:' + GameDataTJDDZ.roomsummaryData.maxFen[player.uid];
            }
            if (Object.keys(GameDataTJDDZ.roomsummaryData.maxBoom).length != 0) {
                maxBobm.getComponent("cc.Label").string = '打出炸弹数:' + GameDataTJDDZ.roomsummaryData.maxBoom[player.uid];
            }
        }
        ...
        if(gameDefine.currencyType.Currency_Coin == GameData.room.opts.currencyType){
            ...
            if (GameDataTJDDZ.roomsummaryData.score != null) {
                ...
                var GetScoreMaxUid = function () {
                    ...
                    for (let i = 1; i < GameData.joiners.length; i++) {
                        if (GameData.joiners[i] && GameDataTJDDZ.roomsummaryData.score[GameData.joiners[i].uid] > GameDataTJDDZ.roomsummaryData.score[tempUid]) {
                            tempUid = GameData.joiners[i].uid;
                        }
                    }
                    return tempUid;
                };
                if (GameDataTJDDZ.roomsummaryData.score[GetScoreMaxUid()] == GameDataTJDDZ.roomsummaryData.score[player.uid]) {
                    //winNode.active = (GameDataTJDDZ.roomsummaryData.score[GetScoreMaxUid()] > 0);
                }
            }
        }
