cc.Class({
    ...
    onLoad: function () {
        //cc.log('GameData.room.opts.joinermax1 ' + JSON.stringify(GameData.room));
        if (GameData.room) {
    ...
    ShowsPlayerImageOnItme: function (index, playIma) {
        if (playIma == undefined || playIma == '' || playIma.length <= 0) {
            return;
        }
        var self = this;
    ...
    ShowPlayerScoreItem: function () {
        ...
        //cc.log('GameData.room.opts.joinermax2 ' + GameData.room.opts.joinermax);
        if (this.playerNum != undefined) {
            this.playerNum = GameData.room.opts.joinermax;
            this.AddSumScore();
        }
