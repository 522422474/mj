cc.Class({
    ...
    showPlayer: function (player, idx) {
        ...
        if (GameDataPDK.roomsummaryData && Object.keys(GameDataPDK.roomsummaryData).length != 0) {
            if (Object.keys(GameDataPDK.roomsummaryData.maxFen).length != 0) {
                maxScore.getComponent("cc.Label").string = '单局最高分:' + GameDataPDK.roomsummaryData.maxFen[player.uid];
            }
            if (Object.keys(GameDataPDK.roomsummaryData.maxBoom).length != 0) {
                maxBobm.getComponent("cc.Label").string = '打出炸弹数:' + GameDataPDK.roomsummaryData.maxBoom[player.uid];
            }
        }
        ...
        if (GameDataPDK.roomsummaryData.score != null) {
            ...
            var GetScoreMaxUid = function () {
                ...
                for (let i = 1; i < GameData.joiners.length; i++) {
                    if (GameData.joiners[i] && GameDataPDK.roomsummaryData.score[GameData.joiners[i].uid] > GameDataPDK.roomsummaryData.score[tempUid]) {
