...
var roomTable = cc.Class({
    ...
    onLoad: function () {
        ...
        require('util').registEvent('onGameStart', this, this.handleGameStart);
        require('util').registEvent('onGameReady', this, this.handleGameReady);
        require('util').registEvent('onGameTurn', this, this.handleTurn);
        require('util').registEvent('playerConnect', this, this.showUI);

        //开局前加飘
        //require('util').registEvent('onStarPiao', this, this.showPiaoLayer);
        // 牌局开始
        require('util').registEvent('onMahjongStartRun', this, this.onShow);
        require('util').registEvent('onRegularCircle', this, this.onShow);
        require('util').registEvent('onHuaDianMahjongReconnecet', this, this.onShow);
        require('util').registEvent('onMahjongRunEnd', this, this.onGameScore);
        require('util').registEvent('onCardHu', this, this.handleHuCard);
        require('util').registEvent('onCardChi', this, this.handleChiCard);
        require('util').registEvent('onCardDis', this, this.handleDisCard);
        require('util').registEvent('onCardPeng', this, this.handlePengCard);
        require('util').registEvent('onCardGang', this, this.handleGangCard);
        require('util').registEvent('onCardTing', this, this.handleTingCard);
        require('util').registEvent('onCardBao', this, this.handleBao);
        require('util').registEvent('dianPao', this, this.handleDianPao);
        require('util').registEvent('onPushErrorMsg', this, this.showErrorMsg);
        require('util').registEvent('onCardBtnStart', this, this.onCardBtnStart);
        require('util').registEvent('onCardBtnMove', this, this.onCardBtnMove);
        require('util').registEvent('onCardBtnEnd', this, this.onCardBtnEnd);
        require('util').registEvent('onCardBtnCancel', this, this.onCardBtnCancel);
        ...
        //显示飘选项
        //if (Object.keys(profileChangChun.PiaoInfo).length > 0  && profileChangChun.PiaoInfo.runState == HuaDian.RunStateEmnum.PIAO) {
        //    this.showPiaoLayer();
        //}
    },
    ...
    onDestroy: function () {
        ...
        unrequire('util').registEvent('onGameStart', this, this.handleGameStart);
        unrequire('util').registEvent('onGameTurn', this, this.handleTurn);
        //unrequire('util').registEvent('onStarPiao', this, this.showPiaoLayer);
        unrequire('util').registEvent('playerConnect', this, this.showUI);
        unrequire('util').registEvent('onMahjongStartRun', this, this.onShow);
        unrequire('util').registEvent('onRegularCircle', this, this.onShow);
        unrequire('util').registEvent('onHuaDianMahjongReconnecet', this, this.onShow);
        unrequire('util').registEvent('onMahjongRunEnd', this, this.showResultLayer);
        unrequire('util').registEvent('onCardHu', this, this.handleHuCard);
        unrequire('util').registEvent('onCardChi', this, this.handleChiCard);
        unrequire('util').registEvent('onCardDis', this, this.handleDisCard);
        unrequire('util').registEvent('onCardPeng', this, this.handlePengCard);
        unrequire('util').registEvent('onCardGang', this, this.handleGangCard);
        unrequire('util').registEvent('onCardBao', this, this.handleBao);
        unrequire('util').registEvent('dianPao', this, this.handleDianPao);
        unrequire('util').registEvent('onPushErrorMsg', this, this.showErrorMsg);
        unrequire('util').registEvent('onCardBtnStart', this, this.onCardBtnStart);
        unrequire('util').registEvent('onCardBtnMove', this, this.onCardBtnMove);
        unrequire('util').registEvent('onCardBtnEnd', this, this.onCardBtnEnd);
        unrequire('util').registEvent('onCardBtnCancel', this, this.onCardBtnCancel);
    },
    showUI: function() {
    ...
    onShow: function () {
        ...
        //显示宝牌
        if (GameData.player[GameData.player.uid].hasCheckedBao) {
            this.showBaoCardImg();
        }
    ...
    handlePass: function (data) {
        if (GameData.game.dataInfo.onGoingUserId) {
            ...
            if (!show) MjHandler_CC.getInstance().requestPass_CC(function (res) { });
        ...
        if (data.act == 'hu') {
            ...
            if (data.card.length <= 1) {
                if (data.card[0][0] instanceof Array && data.card[0].length > 1) {
                    ...
                    MjHandler_CC.getInstance().requestOperation_CC(type,data.card[0][0],null,function (res) {
                    });
                }else{
                    MjHandler_CC.getInstance().requestOperation_CC(type,data.card[0],null,function (res) {
                    });
                }
    ...
    onClickChiActionBtn : function(e){
        ...
        if(data != null)
        {
            MjHandler_CC.getInstance().requestOperation_CC(HuaDian.OPERATION.OPERATION_CHI,data,null,function (res) {
                if (res.code == errorCode.Success) {
                    //self.showEffect('chi');
                }
            });
    ...
    onClickActionSecondaryBtn: function (e) {
        ...
        //cc.log('杠牌id：'+cards);
        if (data != null) {
            MjHandler_CC.getInstance().requestOperation_CC(data.type,cards,null,function (res) {});
            ////播放蛋动画
            //var baoAnimNode = cc.find('down',this.baoAnimationNode);
            //this.playBaoAnimation(baoAnimNode,'dan');
        }
            ...
            for (var i = 0; i < size; i++) {
                ...
                if (i < cards.length) {
                    for (var n = 0; n < 7; n++) {
                        ...
                        //判断吃或者碰
                        var isPeng = false;
                        if (cards[i].cards.length < 4 && profileChangChun.isAllEqual(cards[i].cards) && !cards[i].cardType) {
                            isPeng = true;
                        }

                        //判断明杠
                        var isMingGang = false;
                        if (cards[i].cards.length == 4 && profileChangChun.isAllEqual(cards[i].cards)) {
                            isMingGang = true;
                        }
                        //判断暗杠
                        var isAnGang = false;
                        if (cards[i].cards.length == 5 && profileChangChun.isAllEqual(cards[i].cards)) {
                            isAnGang = true;
                        }
                        ...
                        if (n < cards[i].cards.length) {
                            ...
                            if (isPeng) {
                                ...
                                //var index = profileChangChun.getCardIndexByUid(player.uid,uid);
                                //if ((n+1) == index) {
                                ...
                                for (index; index<cards[i].cards.length; index++) {
                                    if(array3[n] == cards[i].cards[index]) {//cards[i].cards[index] == cards[i].cards[index+1] &&
                                        if(cards[i].cards.indexOf(array3[n]) == index) {
                                            continue;
                                        }
                                        //处理
                                        if(n == 1 && (cards[i].cards[0] == cards[i].cards[1] || cards[i].cards[1] == cards[i].cards[2])) {
                                            continue;
                                        }
                                        num ++;
                                    }
...
//----------------------------------------------------特殊情况处理-----------------------------------------------------------------//
    ...
    onHandCardClicked: function (data) {
        ...
        if (this.selectedCard == index) {
            ...
            if (profileChangChun.getPlayerOperationsByUid(GameData.player.uid).indexOf(10) == -1){
                this.isTing = false;
            }else{
                this.isTing = true;
            }
