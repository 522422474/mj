...
cc.Class({
    ...
    onLoad: function () {
        //require('util').registEvent('onShowResult', this, this.onShow);
        //this.RuleHandler = new RuleHandler();
    },

    onDestroy: function () {
        //delete this.RuleHandler;
        //unrequire('util').registEvent('onShowResult', this, this.onShow);
    },
    onShow: function () {
   ...
   showBaoNode: function () {
    ...
    showDetail: function (index) {
        ...
        if (isZhuang && ishu) {
            ...
            label.string += '庄家*2' +' ';
        }else if(isZhuang && profileChangChun.getHuType() == 3){
            label.string += '庄家*2' +' ';
        }
        //杠+分
        ...
        for (var key in selfGang) {
            selfAddGangScore += profileChangChun.getGangSorce(selfGang[key].cards) * (GameData.joiners.length - 1);
        }
        if (selfAddGangScore != 0) {
            label.string += ' 蛋:' + '+' + selfAddGangScore +'分 ';
        }
        //杠-分
        selfDelGangScore = selfAddGangScore - resultStr.scoreList[1];
        if (selfDelGangScore != 0) {
            label.string += ' 蛋:' + '-' + selfDelGangScore +'分 ';
        }
        ...
        // }
        if (!ishu) {
            let strInfo = GameData.cards[player.uid].hasCheckedBao ? "看宝" : "未看宝";
    ...
    showRoundInfo: function () {
        ...
        var time = profileChangChun.resultData.timeStamp * 1000;
        time = new Date(time);
