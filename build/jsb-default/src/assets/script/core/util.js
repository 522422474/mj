...
function createUIPrefab(prefab, parent, position, func) {
...
function createUIPrefabArray(prefabArray, parent, func){
...
function commondMessageBox(content, ok_func, cancel_func) {
    var onResLoaded = function (err, res) {
        var point = cc.director.getWinSize();
        var messagebox = cc.instantiate(res);
        messagebox.getComponent('messagebox').setContent(content);
...
function createMoveMessage(content, code) {
    var onResLoaded = function (err, res) {
        var point = cc.director.getWinSize();
...
function niuniuCreateMoveMessage(content) {
    var onResLoaded = function (err, res) {
        var point = cc.director.getWinSize();
        var messageNode = cc.instantiate(res);
...
function screenPartShoot(node, fun) {
    if (!cc.sys.isNative) return;
...
//获取匹配金币场总等级数
function getMatchSumLv(gameType){
    var configMgr = require('configMgr');
...
//获取百人牛牛上庄条件
function getMatchShangZhuangFinal(gameType, scoreLv){
    var configMgr = require('configMgr');
    if(configMgr == undefined
        || gameType == undefined
        || scoreLv == undefined){
...
//获取游戏类型名字
function getGameTypeNameString(gameType){
    var gameDefine = require('gameDefine');
    var str = '';
    if(gameDefine == undefined){
        return str;
...
//判断是否需要救济金
function IsNeedSuccour(gameType, scoreLv){
...
//检查是否打开救济金界面
function checkOpenUISuccour(gameType, scoreLv){
    var gameDefine = require('gameDefine');
    if(gameDefine == undefined
        || gameType == undefined
        || scoreLv == undefined){
...
//金币数量简洁显示
function ConversionCoinValue(value, digit){
    if(value >= 100000000){
...
//判断日期是不是同一天
function ComparingDateInSameDay(date1, date2){
...
//分享房间口令
function wxShareCommond(content){
...
function showSpineEffect(node, url, animate) {
