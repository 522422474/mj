...
GameData.getOtherHuier = function (huier) {
  if(GameData.room.opts.huierModel == 1){
    return 0;
  }
  if (huier == 9) return 1;
...
GameData.initCreateRoomOpts = function () {
  ...
  GameData.createRoomOpts.huierModel = 0;     //四个混儿
  GameData.createRoomOpts.doubleGang = false; //杠翻番
  GameData.createRoomOpts.jGangScore = 8;     //金杠分数
  GameData.createRoomOpts.laType = 2;         //坐二拉一:1/连庄拉庄:2
...
function registAllMessage() {
  ...
  GameNet.getInstance().setCallBack('onPlayerUpdate', function (data) {
    WriteLog("do event onPlayerUpdate " + JSON.stringify(data));
    GameData.player.card += data.num;
    sendEvent('onPlayerUpdate');
  });
  ...
  GameNet.getInstance().setCallBack('playerAttr', function(data){
    ...
    for (var i = 0, len = fields.length; i < len; ++i) {
      var field = fields[i];
      GameData.player[field] = data[field];
    }
    sendEvent('onPlayerUpdate');
  });
  GameNet.getInstance().setCallBack('playerMissionData', function(data){
    GameData.player.mission = data.data;
    sendEvent('playerMissionData');
  });
}