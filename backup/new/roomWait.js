...
cc.Class({
    ...
    backBtnClicked: function () {
        if (GameData.room.creator == GameData.player.uid) {
            ...
            RoomHandler.getInstance().quitRoom(GameData.room.id);
        }
    },
    ...
    getInviteStr: function () {
        // var playerid = "玩家ID:" + GameData.player.uid;
        var str1 = "玩法:"
        var playStr = getRoomRuleStr(GameData.room.opts);
        // var longType = ",拉五龙";
        // if (GameData.room.opts.long) {playStr += ",拉五龙";}else {playStr += ",不带拉五龙";}
        // var longType = ",拉五龙";
        // if (GameData.room.long) {playStr += ",拉五龙";}else {playStr += ",不带拉五龙";}
        // var difen = ",1倍底分";
        // if (GameData.room.opts.times == 1) {difen = ",1倍底分";}
        // else if (GameData.room.opts.times == 2) {difen = ",2倍底分";}
        // else if (GameData.room.opts.times == 5) {difen = ",5倍底分";}

        var str2 = ",请您快速加入对局.";
        var des = str1 + playStr + str2;
        return des;
    },
    ...
    onDeleteRoom: function () {
        createMessageBox('解散房间不扣房卡，是否确定解散？', function () {
            RoomHandler.getInstance().deleteRoom(GameData.room.id, 'close');
        }, function () {});
    },
