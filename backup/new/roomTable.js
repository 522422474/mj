...
var roomTable = cc.Class({
    ...
    onLoad: function () {
        ...
        require('util').registEvent('onGamePass', this, this.handlePass);
        require('util').registEvent('initCards', this, this.onShow);
        //require('util').registEvent('initZhuangInfo', this, this.showPlayers);
        require('util').registEvent('onCardObtain', this, this.handleObtain);
        ...
        require('util').registEvent('onHandCardClicked', this, this.onHandCardClicked);
        ...
        require('util').registEvent('onGameScore', this, this.onGameScore);
//        require('util').registEvent('onJoinerLost', this, this.showJoinerLost);
 //       require('util').registEvent('onJoinerConnect', this, this.showJoinerConnect);
        ...
        //this.schedule(function(){this.node.emit('onRoomInfo');}, 1); //1秒更新一次
    ...
    onDestroy: function() {
        ...
        unrequire('util').registEvent('onGamePass', this, this.handlePass);
        unrequire('util').registEvent('initCards', this, this.onShow);
        //unrequire('util').registEvent('initZhuangInfo', this, this.showPlayers);
        unrequire('util').registEvent('onCardObtain', this, this.handleObtain);
        ...
        unrequire('util').registEvent('onHandCardClicked', this, this.onHandCardClicked);
        ...
        unrequire('util').registEvent('onGameScore', this, this.onGameScore);
    ...
    showCards: function() {
        ...
        if (player && GameData.cards[player.uid]) {
            ...
            this.7890-(player, this.play_layer_down);
    ...
    moveBackMyCards : function()
    {
        for(var i = 1; i <= GameData.client.handsize; i++)
        {
           ...
           //      if( cardButton.getIndex() == this.operation_card ) {
           //          continue;
           //      }
           //  }
           cardNode.y = 0;
    ...
    showMyHandCards: function(player, parent) {987654``
    ...
    // setCardBtnTouch : function(node){

    //     if( this.operation_card < 0 ) {
    //         return false;
    //     }

    //     var cardBtn = node.getChildByName(node.name + '_button');
    //     if(cardBtn)
    //     {
    //         var cardButton = cardBtn.getComponent('CardButton');
    //         if( cardButton.getIndex() != this.operation_card ) {
    ...
    //     this.first_move_position = data.detail.position;

    //     //检查是否有其他的牌正在执行触摸事件
    //     if( this.setCardBtnTouch(node) == true ) {
    //         cc.log("..return");
    //         return ;
    //     }
    //     //设置正在触摸的牌 index
    //     this.operation_card = index;

    //     //将所有牌复位
    //     //this.setAllHandCardsBack(node);
    // },
    // onCardBtnMove:function(data){
    //     var index = data.detail.index;
    //     var position = data.detail.position;
    //     var node = cc.find('cardHand/cardHand'+index, this.play_layer_down);
    ...
    //     if( Math.abs(position.x - this.first_move_position.x ) >= 10
    //         || Math.abs(position.y - this.first_move_position.y ) >= 10 )
    //     {
    //         this.is_move = true;
    //     }
    ...
    //     //如果超出指定范围，则走滑动操作逻辑，否则，走点击操作逻辑
    //     if( Math.abs(position.x - this.first_move_position.x ) >= 10
    //         || Math.abs(position.y - this.first_move_position.y ) >= 10 )
    //     {
    //         //判断是否滑过指定范围
    //         if( node.getPositionY() < 200 ) {
    //             node.setPosition(this.myHandsInitPosition[index]);
    //             this.selectedCard = -1;
    //             this.moveBackMyCards();
    //             this.tingTip.hide();
    //             return;
        ...
        if (this.selectedCard == index) {
            ...
            if(RuleHandler.instance.isHuier(card)){
                createMessageBox("您是否确认将混儿牌打出，若打出将不能胡任何牌型，请谨慎选择。", function(){
                    MjHandler.getInstance().requestDisCard(card, function(ret) {
                        if (ret.result == ZJHCode.Success) {
                            node.stopAllActions();
                            // node.setPosition(self.myHandsInitPosition[index]);
                            node.y = 0;
                            self.selectedCard = -1;
            ...
            if (this.selectedCard >= 0) {
                var last = cc.find('cardHand/cardHand'+this.selectedCard, this.play_layer_down);
                 // last.setPosition(this.myHandsInitPosition[this.selectedCard]);
                 last.runAction(cc.moveTo(0.01, cc.p(last.getPosition().x, 0)));
            }
            node.runAction(cc.moveTo(0.01, cc.p(node.getPosition().x, 30)));
            // node.setPosition(cc.p(this.myHandsInitPosition[index].x,this.myHandsInitPosition[index].y+30));
            this.selectedCard = index;
            if(GameData.game.turn == GameData.player.uid)
            {
    ...
    //     this.selectedCard = index;
    //     this.limit = 100;
    //     var p = node.parent.convertToNodeSpaceAR(position);
    //     node.setPosition(p);
    //     if (Math.abs(p.x-this.myHandsInitPosition[index].x)<42 && Math.abs(p.y - this.myHandsInitPosition[index].y)<68) {
    //         // if (p.y == 0 || p.y < 30) {
    //             node.setPosition(this.myHandsInitPosition[index].x, 30);
    //             this.selectedCard = -1;
    //             this.moveBackMyCards();
    //             return;
    //         // }
